...
$.fn.extend({ scrambler:
    ...
    function(square_size) {
        ...
        $(game_object_element).css({height: board_size, width: board_size});

        $(game_object_element).html('<div id="board"></div>');

        $('#board').children('div').css({height: sq_size, width: sq_size, lineHeight: sq_size, backgroundSize: board_with_padding + " " + board_with_padding});

        $('#board').css({ position: 'absolute', width: board_size, height: board_size, border: '1px solid gray'});

        for(var i = 0; i < 16; i++) {
            $('#board').append("<div style='left:" + ((i % 4) * square_size) + "px; top: " + Math.floor(i / 4) * square_size + "px; width: "
        ...
        $('#board').children("div:nth-of-type(" + empty_square + ")").css({backgroundImage: "", background: "#ffffff"}); 
...
function unShuffle(square_size){
    for(var i = 1; i <= 16; i++) {
        ...
        $("#board div:nth-of-type(" + i + ")").css({left: left, top: top});
    }
}
...
function win(square_size){
    ...
    $("#easy").css({backgroundImage: "", background: "#000000"});
}
...
function getImages(square_size) {
    ...
    $('#board').children('div').css({backgroundImage: "url('" + g_image_for_overlay + "')"}); 
    $('#board').children("div:nth-of-type(" + empty_square + ")").css({backgroundImage: "", background: "#ffffff"}); 
    $('.image').css({backgroundImage: "url('" + g_image_for_overlay + "')", width: (square_size * 4) + "px", height: (square_size * 4) + "px"});
}
...
function Move(clicked_square, square_size, do_animate) {
    ...
    if (movable) {
        ...
        if(do_animate) {
            $(clicked_square).animate({ left: empty_x, top: empty_y }, 200, function(){
